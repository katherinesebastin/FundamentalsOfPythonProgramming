...
class Reservation:
    def __init__(self, reservation_id: int, name: str, email: str, phone: str, date: date, time: time, duration: int, price: float, confirmed: bool, resource: str, created: datetime):
...
def convert_reservation_data(reservation: list[str]) -> Reservation:
    """
    Convert a list of reservation fields into a Reservation object.

    Parameters:
     reservation (list): Unconverted reservation -> 11 columns 

    Returns:
     converted (reservation): Converted data types
    """
    return Reservation(
        ...
        email = reservation[2],
        phone = reservation[3],
        date = datetime.strptime(reservation[4], "%Y-%m-%d").date(),
        time = datetime.strptime(reservation[5], "%H:%M").time(),
...
def fetch_reservations(reservation_file: str) -> list[Reservation]:
    """
    Reads reservations from a file and returns the reservations
...
def confirmation_statuses(reservations: list[Reservation]) -> None:
    ...
    """
    for reservation in reservations:
        print(f"{reservation.name} → {'Confirmed' if reservation.is_confirmed() else 'NOT Confirmed'}")
...
def confirmation_summary(reservations: list[Reservation]) -> None:
    ...
    Parameters:
     reservations (list): Reservations
    """
    confirmed_count = len([reservation for reservation in reservations if reservation.is_confirmed()])
...
def total_revenue(reservations: list[Reservation]) -> None:
    ...
    Parameters:
     reservations (list): Reservations
    """
    revenue = sum(reservation.total_price() for reservation in reservations if reservation.is_confirmed())
